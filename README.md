...
| Cores, fontes, espaçamentos | `css/style.css`, seção `:root` no topo — paleta branco/rosa/dourado (`--marigold` = rosa, `--gold`/`--moss` = dourado, `--loam` = painéis de destaque em gradiente) |
| Itens do menu de navegação (dock nav) | Repetido em todas as páginas, dentro de `<nav class="dock-nav">` — cada item é um `<a class="dock-item">` com ícone SVG e texto |
...
### Sobre a navegação (dock nav animada)

O menu principal é uma barra "dock" com um indicador em gradiente (rosa → dourado)
que desliza suavemente atrás do item em destaque — acompanha o mouse ao passar por
cima e volta para a página atual quando o mouse sai. Em telas pequenas, a barra vira
um dock fixo na parte inferior da tela, só com ícones (padrão comum em apps mobile).

A lógica de animação está em `js/main.js`, na seção "Dock nav — indicador deslizante".
Para adicionar uma nova página ao menu, copie um bloco `<a class="dock-item">...</a>`
em **todas** as páginas (o menu é repetido manualmente em cada arquivo, não há
templates) e marque `class="dock-item active"` apenas no item da página atual.
